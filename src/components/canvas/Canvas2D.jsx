...
export default function Canvas2D({ grid = true, axes = true }) {
  ...
  useEffect(() => {
    ...
    const cs = getComputedStyle(document.body); const gf = cs.getPropertyValue('--grid-fine').trim()||'rgba(128,128,128,0.06)'; const gm = cs.getPropertyValue('--grid-main').trim()||'rgba(128,128,128,0.12)'; const ga = cs.getPropertyValue('--axis').trim()||'rgba(128,128,128,0.2)'
    const rawStep = zoom > 20 ? 0.1 : zoom > 10 ? 0.2 : zoom > 5 ? 0.25 : zoom > 2 ? 0.5 : zoom > 1 ? 1 : zoom > 0.5 ? 2 : zoom > 0.2 ? 5 : 10
    if (grid) { const fs = rawStep / 5; ctx.strokeStyle = gf; ctx.lineWidth = 0.3; for (let g = Math.floor((ox - R) / fs) * fs; g <= ox + R; g += fs) { const px = cx + (g - ox) * s; if (px >= 0 && px <= W) { ctx.beginPath(); ctx.moveTo(px, 0); ctx.lineTo(px, H); ctx.stroke() } }; for (let g = Math.floor((oy - R) / fs) * fs; g <= oy + R; g += fs) { const py = cy - (g - oy) * s; if (py >= 0 && py <= H) { ctx.beginPath(); ctx.moveTo(0, py); ctx.lineTo(W, py); ctx.stroke() } }; ctx.strokeStyle = gm; ctx.lineWidth = 0.5; for (let g = Math.floor((ox - R) / rawStep) * rawStep; g <= ox + R; g += rawStep) { const px = cx + (g - ox) * s; if (px >= 0 && px <= W) { ctx.beginPath(); ctx.moveTo(px, 0); ctx.lineTo(px, H); ctx.stroke() } }; for (let g = Math.floor((oy - R) / rawStep) * rawStep; g <= oy + R; g += rawStep) { const py = cy - (g - oy) * s; if (py >= 0 && py <= H) { ctx.beginPath(); ctx.moveTo(0, py); ctx.lineTo(W, py); ctx.stroke() } } }
    const hasNoVisible = visible.length === 0
    const axisX = cx - ox * s; const axisY = cy + oy * s
    if (axes) { ctx.strokeStyle = ga; ctx.lineWidth = 1; if (hasNoVisible) { ctx.setLineDash([6, 5]); ctx.strokeStyle = cs.getPropertyValue('--text-muted').trim() || 'rgba(255,255,255,0.2)' }; if (axisY >= 0 && axisY <= H) { ctx.beginPath(); ctx.moveTo(0, axisY); ctx.lineTo(W, axisY); ctx.stroke() }; if (axisX >= 0 && axisX <= W) { ctx.beginPath(); ctx.moveTo(axisX, 0); ctx.lineTo(axisX, H); ctx.stroke() }; ctx.setLineDash([]) }
    if (axes) { ctx.fillStyle = hasNoVisible ? 'var(--text-muted)' : 'rgba(255,255,255,0.25)'; ctx.font = '9px var(--font-mono)'; ctx.textAlign = 'center'; for (let t = Math.floor((ox - R) / rawStep) * rawStep; t <= ox + R; t += rawStep) { if (Math.abs(t) < rawStep / 2) continue; const px = cx + (t - ox) * s; if (px >= 0 && px <= W) ctx.fillText(Number.isInteger(t) ? t : t.toFixed(rawStep < 1 ? 1 : 0), px, axisY + 14) }; for (let t = Math.floor((oy - R) / rawStep) * rawStep; t <= oy + R; t += rawStep) { if (Math.abs(t) < rawStep / 2) continue; const py = cy - (t - oy) * s; if (py >= 0 && py <= H) ctx.fillText(Number.isInteger(t) ? t : t.toFixed(rawStep < 1 ? 1 : 0), axisX - 18, py + 4) } }
